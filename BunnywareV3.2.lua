...
local fov = 90
local offset = 2.0
local aimbotPredictionOffset = 1.0 -- NEW: predictive lead offset
local fovCircleTransparency = 0.3 -- default
...
-- NEW: Prediction Offset for aimbot (separate from shooting)
AimbotTab:CreateSlider({
...
CombatTab:CreateParagraph({
    Title = "Tip",
    Content = "Other known good offsets are 1.2, 1.4, 2.7 (Default is 2.0)"
})
...
local function AimAtVisual(target)
    ...
    local vel = hrp.AssemblyLinearVelocity
    local predictedPos = currentPos + vel * (aimbotPredictionOffset / 15) -- same formula as shooting

    -- Clamp prediction to avoid overshoot (optional)
    local targetCF = CFrame.new(Camera.CFrame.Position, predictedPos)

    -- Smoothness: lerp factor = 1 - smoothness (0 = instant, 1 = very slow)
    local factor = 1 - smoothness
    -- Ensure factor is not too low to avoid jitter (clamp to 0.01)
...
heartbeatConnection = RunService.Heartbeat:Connect(function(deltaTime)
    ...
    -- Misc features (speed, jump, fly, noclip)
