...
-- Teleport to Dropped Gun (with return teleport after picking up)
...
MainTab:CreateButton({
    ...
    Callback = function()
        ...
        if char and char:FindFirstChild("HumanoidRootPart") then
            ...
            -- Check if gun was picked up (look for it in player's character or backpack)
            local gotGun = false
...
MainTab:CreateToggle({
    ...
    Description = "Automatically teleports to and picks up dropped gun.",
    CurrentValue = false,
    Callback = function(v)
        autoGetGun = v
        if v then
...
-- ============================================================
-- ESP TAB (Coins removed, auto-refresh added)
-- ============================================================
...
-- Auto-refresh ESP on new round (when RoundTimerPart changes)
local function onRoundStart()
    task.wait(2)
    refreshESP()
    print("ESP refreshed for new round.")
end

-- Watch for round timer changes to detect new round
...
-- Also watch for player additions
...
-- ============================================================
-- COMBAT TAB (with floating buttons)
-- ============================================================
...
-- Throw Knife Now (with floating button support)
CombatTab:CreateParagraph({
    Title = "Knife Throw",
    Text = "Hold-click a button to create a floating button."
})

CombatTab:CreateButton({
    Name = "Throw Knife Now",
    Description = "Throws knife at nearest player (manual, uses prediction offset). Hold-click for floating button.",
    Callback = function()
...
-- Silent Shoot (with floating button support)
CombatTab:CreateButton({
    Name = "Silent Shoot (Murderer)",
    Description = "Shoots the murderer with prediction, without moving your camera. Hold-click for floating button.",
    Callback = function()
        local localPlayer = game.Players.LocalPlayer
...
-- Fling (fixed - now properly flings players)
CombatTab:CreateParagraph({
    Title = "Fling",
    Text = "Launch other players (hold-click for floating button)."
})

local function fixedFling(playerToFling)
    ...
    -- Teleport target to player's position + slight offset
...
CombatTab:CreateButton({
    ...
    Callback = function()
        local murderer = findMurderer()
        if murderer then fixedFling(murderer) else print("No murderer found.") end
    end
})
...
CombatTab:CreateButton({
    ...
    Callback = function()
        ...
        if sheriff then fixedFling(sheriff) else print("No sheriff found.") end
    end
})
    ...
    if not u74 then
        ...
        if not targetCoin then
            if flingMurdererAfterFarm then
                local murd = findMurderer()
                if murd then fixedFling(murd) end
            end
            u64(returnLocation)
            return
        end
...
local function onGameStart()
    task.wait(1)
    ReadyUp()
    -- Refresh ESP when new round starts
    task.wait(0.5)
    refreshESP()
end
...
-- Refresh ESP periodically to catch new players (every 5 seconds)
task.spawn(function()
    while true do
        task.wait(5)
        if espEnabled then
